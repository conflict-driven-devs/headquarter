...
func SetupRoutes(router *mux.Router, authHandlers *handler.AuthHandler, instanceHandler *handler.InstanceHandler, projectHandlers *handler.ProjectHandler, webHandler *handler.WebHandler, userHandlers *handler.UserHandler, setupHandler *handler.SetupHandler, profileHandler *handler.ProfileHandler, analyticsHandler *handler.AnalyticsHandler, apiKeyHandler *handler.APIKeyHandler, db *gorm.DB) {
	...
	router.HandleFunc("/scripts/setup.sh", func(w http.ResponseWriter, r *http.Request) {
		content, err := os.ReadFile("infra/scripts/setup.sh")
		if err != nil {
			utils.WriteError(w, http.StatusNotFound, "setup script not available")
			return
		}
	...
	// bootstrap token creation (admin)
	instanceRouter.HandleFunc("/{id}/bootstrap-token", handler.NewBootstrapHandler(db).CreateToken).Methods("POST")

	// /api/bootstrap - used for server bootstrap registration (uses HQ_BOOTSTRAP_KEY)
	bootstrapHandler := handler.NewBootstrapHandler(db)
	apiRouter.HandleFunc("/bootstrap", bootstrapHandler.Register).Methods("POST")
